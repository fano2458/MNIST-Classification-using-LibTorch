...
struct Net : torch::nn::Module {
  Net()
	  : conv1(torch::nn::Conv2dOptions(1, 10, 5)),
        ...
        conv3(torch::nn::Conv2dOptions(20, 30, 3)),
		fc1(120, 50),
		fc2(50, 10) {
  ...
  torch::Tensor forward(torch::Tensor x) {
	x = torch::gelu(torch::max_pool2d(conv1->forward(x), 2));
	x = torch::gelu(torch::max_pool2d(conv2->forward(x), 2));
    // std::cout << x.sizes() << std::endl;
    x = torch::gelu(conv3->forward(x));
    // std::cout << x.sizes() << std::endl;
	x = x.view({-1, 120});
...
void train(size_t epoch, Net& model, torch::Device device, DataLoader& data_loader, 
